...
def calculate_confusion_matrix(output_file):
    """
    Reads the output CSV generated by the Ollama classifier and calculates
    the confusion matrix using real_class and new_class.
    ...
    print(classification_report(y_true, y_pred, labels=metric_labels))

    print("\nPer-label metrics:")
    for label, stats in metrics["per_label"].items():
        print(
            f"{label}: precision={stats['precision']:.4f}, recall={stats['recall']:.4f}, "
            f"f1={stats['f1_score']:.4f}, support={stats['support']}"
        )
